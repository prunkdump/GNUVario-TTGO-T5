...
// Sound class routines

XT_Sound_Class::XT_Sound_Class():XT_Sound_Class(INSTRUMENT_ORGAN,127)
{
	  // See main constructor routine for description of passed parameters
	  // and defaults
}
...
XT_Sound_Class::XT_Sound_Class(int16_t InstrumentID):XT_Sound_Class(InstrumentID,127)
{
	  // See main constructor routine for description of passed parameters
	  // and defaults
}
...
XT_Sound_Class::XT_Sound_Class(int16_t InstrumentID, uint8_t Volume)
{
	// Volume : Volume of sound 0- 127 (max)
	this->Note=abs(NOTE_C4);					// default note
	this->SoundDuration=1000;					// default note length, ignored if using envelopes
...
void XT_Sound_Class::SetInstrument(uint16_t Instrument)
{
	// To create your new "permanent" instrument to the code, add a line here that calls its
	// set up routine and add this as a private function to the header file for the
	// Instrument class. Also add a suitable # define to the Instruments.h header file
...
void XT_Sound_Class::SetDefaultInstrument()
{
	SetWaveForm(WAVE_SQUARE);
}


void XT_Sound_Class::SetPianoInstrument()
{
	SetWaveForm(WAVE_TRIANGLE);
	FirstEnvelope=new XT_Envelope_Class();
	FirstEnvelope->AddPart(25,127);
...
void XT_Sound_Class::SetHarpsichordInstrument()
{
	SetWaveForm(WAVE_SAWTOOTH);
	FirstEnvelope=new XT_Envelope_Class();
	FirstEnvelope->AddPart(15,127);
...
void XT_Sound_Class::SetOrganInstrument()
{
	SetWaveForm(WAVE_SINE);
	FirstEnvelope=new XT_Envelope_Class();
	FirstEnvelope->AddPart(15,127);
...
void XT_Sound_Class::SetSaxophoneInstrument()
{
	SetWaveForm(WAVE_SQUARE);
	FirstEnvelope=new XT_Envelope_Class();
	FirstEnvelope->AddPart(15,127);
...
void XT_Sound_Class::SetWaveForm(uint8_t WaveFormType)
{
	// Sets the wave form for this instrument
...
void XT_Sound_Class::SetFrequency(uint16_t Freq)
{
	// only if not using note property
	Note=-1;
	WaveForm->Frequency=Freq;
...
uint8_t XT_Sound_Class::NextByte()
{
	uint8_t ByteToPlay;
...
//	if (WaveForm->Playing == false) Playing=false;
...
XT_Beeper_Class::XT_Beeper_Class(int16_t* Beep):XT_Beeper_Class(Beep,TEMPO_ALLEGRO,&DEFAULT_INSTRUMENT)
{
	// set tempo and instrument to default, number of plays to 1 (once)
}


XT_Beeper_Class::XT_Beeper_Class(int16_t* Beep,uint16_t Tempo):XT_Beeper_Class(Beep,Tempo,&DEFAULT_INSTRUMENT)
{
	// set instrument to default and plays to 1
}


XT_Beeper_Class::XT_Beeper_Class(int16_t* Beep,uint16_t Tempo,XT_Instrument_Class* Instrument)
{
	// Create music score
	this->Beep=Beep;
	this->Tempo=Tempo;
...
XT_Beeper_Class::XT_Beeper_Class(int16_t* Beep,uint16_t Tempo,uint16_t InstrumentID)
{
	// Create music score
	this->Beep=Beep;
	this->Tempo=Tempo;
...
void XT_Beeper_Class::Init()
{
	...
	ChangeNoteEvery=(60/float(Tempo))*BytesPerSec;
	ChangeNoteCounter=0;					// ensures starts by playing first note with no delay
	BeepIdx=0;								// set position to first no
}


uint8_t XT_Beeper_Class::NextByte()
{
	// returns next byte for the DAC
	...
	if(ChangeNoteCounter==0)  {
		// Yes, new note
		if(Beep[BeepIdx]==SCORE_END)  // end of data
		{
			Playing=false;
			return 0;    	// return silence
		}

		Instrument->SetFrequency(abs(Beep[BeepIdx]));			// only if not using note property

//		Instrument->Note=abs(Beep[BeepIdx]);			// convert the negative value to positive index.
		BeepIdx++;										// move to next note
...
/*uint8_t XT_Beeper_Class::NextByte()
{
	...
	if(ChangeNoteCounter==0)  {
		...
		// set length of play for instrument
		// Check next data value to see if it is a beat value
		if(Beep[BeepIdx]>0) 							// positive value, therefore not default beat length
		{
			// Set the duration, beat value of 1=0.25 beat, 2 0.5 beat etc. So just divide by 4
			// to get the real beat length,
			Instrument->Duration=(ChangeNoteEvery*(float(Beep[BeepIdx])/4));
			// Then times by 0.8 to allow for natural movement
			// of players finger on the instrument.
			Instrument->SoundDuration=Instrument->Duration*0.8;
			ChangeNoteCounter=Instrument->Duration;      	// set back to start of count ready for next note
			BeepIdx++;										// point to next note, ready for next time
		}
		else
		{
			// default single beat values
...
void XT_Beeper_Class::SetInstrument(uint16_t InstrumentID)
{
	Instrument->SetInstrument(InstrumentID);
}
