 ...
 *
 */
int mpu_get_biases(long *gyro, long *accel, unsigned char hw_test)
{
 unsigned char data[MAX_PACKET_LENGTH];
 unsigned char packet_count, ii;
 unsigned short fifo_count;
 ...
 accel[0] = (long)(((long long)accel[0]) / test.accel_sens /  packet_count);
 accel[1] = (long)(((long long)accel[1]) / test.accel_sens / packet_count);
 accel[2] = (long)(((long long)accel[2]) / test.accel_sens / packet_count);
...
int fastMPUWriteMem(uint16_t memAddr, uint16_t length, const uint8_t *data) {
  ...
  com[1] = (uint8_t)(memAddr & 0xFF);
	...
	if (i2c_write(st.hw->addr, st.reg->bank_sel, 2, com)) return -1;
//  intTW.writeBytes(INV_HW_ADDR, INV_REG_BANK_SEL, 2, com);

  /* write data */
  //intTW.writeBytes(INV_HW_ADDR, INV_REG_MEM_R_W, length, (uint8_t *)data);
	if (i2c_write(st.hw->addr, st.reg->mem_r_w, length, (uint8_t *)data)) return -1;
...
int fastMPUSetGyroBias8(const uint8_t* bias) {
...
/******************************/
int fastMPUSetAccelBias8(const uint8_t* bias) {
