...
/*    1.0    11/09/19   Ajout gestion des pins SDA / SCL                         */
/*                            inv_error_t begin(int SDA_Pin, int SCL_Pin);  		 */
/*											Ajout fonctions de calibration et compensation           */
/*    1.0.1  15/09/19   Ajout Read/write EEprom                                  */
/*                                                                               */
/*********************************************************************************/
...
inv_error_t MPU9250_DMP::begin(int SDA_Pin, int SCL_Pin)
{
	inv_error_t result;
    struct int_param_s int_param;
...
inv_error_t MPU9250_DMP::set_gyro_biases(long *gyro_bias)
{
...
inv_error_t MPU9250_DMP::get_gyro_biases(long *gyro_bias)
{
...
inv_error_t MPU9250_DMP::get_accel_biases(long *accel_bias)
{
...
inv_error_t MPU9250_DMP::updateCal(unsigned char sensors)
{
	...
	if (sensors & UPDATE_GYRO)
		gErr = updateCalGyro();
	if (sensors & UPDATE_COMPASS)
		mErr = updateCalCompass();
	if (sensors & UPDATE_TEMP)
		tErr = updateCalTemperature();
	
	return aErr | gErr | mErr | tErr;
}

int MPU9250_DMP::updateCalAccel(void)
{
	short data[3];
	...
	ax = data[X_AXIS] + accelBias[0];
	ay = data[Y_AXIS] + accelBias[1];
	az = data[Z_AXIS] + accelBias[2];
	return INV_SUCCESS;
}

int MPU9250_DMP::updateCalGyro(void)
{
	short data[3];
	...
	gx = data[X_AXIS] + gyroBias[0];
	gy = data[Y_AXIS] + gyroBias[1];
	gz = data[Z_AXIS] + gyroBias[2];
	return INV_SUCCESS;
}

int MPU9250_DMP::updateCalCompass(void)
{
	short data[3];
...
inv_error_t MPU9250_DMP::updateCalTemperature(void)
{
	return mpu_get_temperature(&temperature, &time);
}
...
void MPU9250_DMP::ReadEeprom() {
  int eepromAddress = CAL_MPU_HEADER_EEPROM_ADDRESS;
  int val_int = 0;
...
//  eepromAddress = EEPROMAnythingRead(eepromAddress, reinterpret_cast<char*>(&val_int), sizeof(val_int));
  ...
  SerialPort.println(val_int); 
	SerialPort.print("CAL_MPU_EEPROM_TAG : ");
	SerialPort.println(CAL_MPU_EEPROM_TAG);
#endif //EEPROM_DEBUG
 
  if (val_int == CAL_MPU_EEPROM_TAG) {  
...
void MPU9250_DMP::WriteEeprom() {
  ...
  // int to EEPROM
  int val_int = CAL_MPU_EEPROM_TAG;
#ifdef EEPROM_DEBUG	
  SerialPort.print("CAL_MPU_EEPROM_TAG : ");	
	SerialPort.println(val_int);
...
//  EEPROMHAL.commit();
  ...
  val_int = (int)accelBias[0];
	EEPROMHAL.writeIntE(eepromAddress, val_int);
  eepromAddress += sizeof(val_int);

  val_int = (int)accelBias[1];
	EEPROMHAL.writeIntE(eepromAddress, val_int);
  eepromAddress += sizeof(val_int);

  val_int = (int)accelBias[2];
	EEPROMHAL.writeIntE(eepromAddress, val_int);
  eepromAddress += sizeof(val_int);

  val_int = (int)gyroBias[0];
	EEPROMHAL.writeIntE(eepromAddress, val_int);
  eepromAddress += sizeof(val_int);

  val_int = (int)gyroBias[1];
	EEPROMHAL.writeIntE(eepromAddress, val_int);
  eepromAddress += sizeof(val_int);

  val_int = (int)gyroBias[2];
	EEPROMHAL.writeIntE(eepromAddress, val_int);
  eepromAddress += sizeof(val_int);
}