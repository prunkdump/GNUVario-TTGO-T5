...
#define GxGDEP015OC1_PU_DELAY 3  //300
...
void GxGDEP015OC1::updateWindowNL(uint16_t x, uint16_t y, uint16_t w, uint16_t h, bool using_rotation)
{
  if (state == STATE_OK) { 
#ifdef SCREEN_DEBUG
...
unsigned int GxGDEP015OC1::GetState(void){
	return state;
}	
...
void GxGDEP015OC1::_waitWhileBusy2(const char* comment)
{
  unsigned long start = micros();
  while (1)
  {
