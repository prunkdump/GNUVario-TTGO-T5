...
void VarioLog::init(void)
{
  ...
  if (SDHAL_SD.exists(tmpFileName))
  {
    ...
    if (EnableDebug)
      SerialPort.println("log : Activé");
    else
      SerialPort.println("log : Desactivé");

    if (EnableSerial)
      SerialPort.println("log Serial : Activé");
    else
      SerialPort.println("log Serial : Desactivé");

    if (EnableSdCard)
      SerialPort.println("log SdCard : Activé");
    else
      SerialPort.println("log SdCard : Desactivé");

    if (EnableDebugESP32)
      SerialPort.println("log Debug : Activé");
    else
      SerialPort.println("log Debug : Desactivé");
  }

  // Ouverture du fichier de log
  if (EnableDebug && EnableSdCard)
    openSd();
}
...
void VarioLog::traitement(uint8_t type, uint8_t module, String msg)
{
  ...
  if (EnableSerial && EnableDebugModule[module])
    DUMP(msg);
  if (EnableSdCard && EnableDebugModule[module])
    writeSd(msg);
  //		send(tmpMessage);
...
//********************************************
{
...
#ifdef SDCARD_DEBUG
      char tmpstr[100];
      sprintf(tmpstr, "openLog: %s error\n", log_name);
      SerialPort.println(tmpstr);
#endif
      return f;
    }
...
#ifdef SDCARD_DEBUG
    char tmpstr[100];
    sprintf(tmpstr, "openLog: %s missing\n", log_name);
    SerialPort.println(tmpstr);
#endif
  ...
  if (f.size() > log_size)
  {
    ...
    logRotate();
#ifdef SDCARD_DEBUG
    sprintf(tmpstr, "openLog: create %s\n", log_name);
    SerialPort.println(tmpstr);
#endif
