...
******************************************************************************/
#ifndef _MPU9250_DMP_SF_EXT_H_
#define _MPU9250_DMP_SF_EXT_H_
...
struct CalMPU {
	int 	axOffset, ayOffset, azOffset;   					//Accelerometre
	int 	gxOffset, gyOffset, gzOffset;							//Gyroscope
	int 	mxOffset, myOffset, mzOffset;							//Magnetometre
	long 	qwOffset, qxOffset, qyOffset, qzOffset;		//DMP
	float tempOffset;																//Temperature
};
...
public:
	...
	inv_error_t readAccelBias(long *accel_bias);

	inv_error_t writeAccelBias(long *accel_bias);

	inv_error_t readGyroBias(long *gyro_bias);

	inv_error_t writeGyroBias(long *gyro_bias);

	inv_error_t get_biases(long *gyro_bias, long *accel_bias);
	
	inv_error_t set_accel_biases(long *accel_bias);
	inv_error_t set_gyro_biases(long *gyro_bias);
	
	inv_error_t get_gyro_biases(long *gyro_bias);
	inv_error_t get_accel_biases(long *accel_bias);
	
	inv_error_t updateCal(unsigned char sensors);
	int updateCalAccel(void);
	int updateCalGyro(void);
	int updateCalCompass(void);
	inv_error_t updateCalTemperature(void);
