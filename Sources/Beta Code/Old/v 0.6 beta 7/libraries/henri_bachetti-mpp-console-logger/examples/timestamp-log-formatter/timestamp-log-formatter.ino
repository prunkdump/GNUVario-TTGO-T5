...
/*
 * timestamp formatter sketch for logging 
 * - console and logs output to Serial
 * - a timestamp in millicecond and the logger name are printed at the begining of each message
...
Logger LOGGER("test");
LogTimestampFormatter formatter(&LOGGER);

void setup()
{
  Serial.begin(115200);
  console_init(&Serial);
  printf("Timestamp formatter logging demo\n");
  LOGGER.init(&Serial);
